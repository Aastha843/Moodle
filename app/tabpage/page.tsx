...
export default function TabPage() {
  ...
  // Load tabs from localStorage on first client render
  ...
  // Persist to localStorage whenever tabs change (after initial load)
  useEffect(() => {
    if (!loaded) return;
  ...
  // Generate INLINE-ONLY output (no classes)
  const outputHtml = useMemo(() => {
    ...
    // Inline JS (simple tab switcher with ARIA)
    const js =
  ...
  return (
    <div>
      ...
      <main className="container">
        <h1 style={{ marginTop: 0 }}>Tabs Page (Up to 15 tabs)</h1>
        <p style={{ marginTop: 0 }}>
          Add (+) and remove (−) tabs, edit headings and contents. Tabs persist to <b>localStorage</b>. Use the
          Output buttons to generate pure HTML (inline CSS only; no classes) that you can paste into <code>Hello.html</code>.
        </p>
